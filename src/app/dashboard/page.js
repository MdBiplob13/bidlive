...
import { Gavel, Heart, ShoppingBag, Bell, Plus, TrendingUp } from "lucide-react";
...
export default function DashboardHome() {
  ...
  const { data: orders } = useQuery({ queryKey: ["orders", "buyer"], queryFn: async () => (await api.get("/orders?role=buyer")).data.data.orders });
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <StatCard icon={ShoppingBag} label={t("nav.orders")} value={orders?.length ?? "—"} href="/dashboard/orders" color="bg-success/10 text-success" />
